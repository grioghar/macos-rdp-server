...
static void context_free(freerdp_peer *peer, rdpContext *ctx) {
    ...
    if (c->gfx) {
        rdpgfx_server_context_free(c->gfx);
        c->gfx = NULL;
    }
    if (c->cliprdr) {
        cliprdr_server_context_free(c->cliprdr);
        c->cliprdr = NULL;
    }
    if (c->rdpsnd) {
        rdpsnd_server_context_free(c->rdpsnd);
        c->rdpsnd = NULL;
    }
    (void)peer;
}
...
static BOOL peer_mouse(rdpInput *input, UINT16 flags,
                        UINT16 x, UINT16 y) {
    RDPPeerContext *ctx = (RDPPeerContext *)input->context;
...
static BOOL peer_mouse_ex(rdpInput *input, UINT16 flags,
                           UINT16 x, UINT16 y) {
    RDPPeerContext *ctx = (RDPPeerContext *)input->context;
...
static UINT gfx_caps_advertise(RdpgfxServerContext *gfx,
    ...
    /* Prefer AVC420 (H.264 baseline) capsV8 */
    ...
    /* Create the primary surface now. */
...
static BOOL peer_post_connect(freerdp_peer *peer) {
    ...
    if (ctx->gfx->Open(ctx->gfx) != CHANNEL_RC_OK) {
        syslog(LOG_WARNING, "GFX channel open failed");
...
bool rdp_peer_send_h264_frame(freerdp_peer *peer,
    ...
    return ctx->gfx->SurfaceCommand(ctx->gfx, &cmd) == CHANNEL_RC_OK;
}
